...
int travel(int sr, int sc, vector<vector<int>> &grid){
   if(sc == grid[0].size()-1){
      return grid[sr][sc];
   }
   ...
   int maxF = INT_MIN;
   if(sc<grid[0].size()-1){
      maxF = travel(sr,sc+1,grid);
   }

   // valid top right single step movement
   int maxUF = INT_MIN;
   if(sc<grid[0].size()-1 and sr>0){
      maxUF = travel(sr-1,sc+1,grid);
   }

   // valid top down single step movement
   int maxDF = INT_MIN;
      if(sc<grid[0].size()-1 and sr<grid.size()-1){
      maxDF = travel(sr+1,sc+1,grid);
   }

   return maxOfThree(maxF,maxDF,maxUF) + grid[sr][sc];

}

int ansEvaluate(vector<vector<int>> &grid){


   // travelling across all the row
   int maxi = INT_MIN;
   for(int i = 0 ; i<grid.size() ; i++){
      int golmineSum = travel(i,0,grid);
