...
void allPath(vector<vector<Edge>> graph,int s, int d , vector <int> psf , vector<bool> visited , int oriS){
    ...
    for(int i = 0 ; i<graph[s].size() ; i++){
        if(visited[graph[s][i].n]==false){
            ...
            allPath(graph,graph[s][i].n,d,psf,visited,oriS);
            psf.pop_back();
        }
    }
...
int main(){
   ...
   vector<int> psf;
    allPath(graph,s,d,psf,visited,s);
